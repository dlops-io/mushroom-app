...
REM sources: https://dockerlabs.collabnix.com/networking/A1-network-basics.html#docker_network; in terminal, run "docker network inspect"
REM The docker network command is the main command for configuring and managing container networks.
REM what this is saying here is to inspect the network (Display detailed information on one or more networks) if the network exists; if it does not exist, then create the network
docker network inspect mushroomappnetwork >NUL || docker network create mushroomappnetwork

REM Run Postgres DB and DBMate
REM sources: https://dockerlabs.collabnix.com/intermediate/workshop/; in Terminal, run "docker-compose run"; https://dockerlabs.collabnix.com/intermediate/workshop/DockerCompose/Difference_between_dockerfile_and_docker_compose.html
...
REM "docker-compose run" means to Run a one-off command on a service
REM "--rm" means to Remove container after run. Ignored in detached mode.
REM "--service-ports" means to Run command with the service's ports enabled and mapped to the host
